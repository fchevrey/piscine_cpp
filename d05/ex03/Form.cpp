...
/*   Updated: 2019/04/01 21:03:56 by fchevrey         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool			Form::execute(const Bureaucrat &executor)
{
	if (!this->_signed)
	{
		throw GradeTooLowException("Form isn't signed");
	}
	if (executor.getGrade() <= this->_ex_grade)
	{
		this->realy_execute();
	}
	else 
		throw GradeTooLowException("Bureaucrat grade too low to execute form") ;
	return true;
}
